...
<?php
		if(!isset($_SESSION['correcto']) ){
			$_SESSION['PAGINA']='Productos.Options.php';
			?>
...
<div id="ladiz"></div>
    ...
    <div id="cuerpo">
    	<div id="barra_vertical">
         ...
         <div id="area_trabajo">
		 <div id="formulario"><!-- InstanceBeginEditable name="formulario" --><b>Agregar Productos</b>
		   <div id="session" style="position: absolute; width: 60px; height: 60px; margin-top: -60px; margin-left: -150px; background-image: url(../Imagenes/salir.gif); background-repeat: no-repeat; background-size: 110% 100%; cursor: pointer; left: 229px; top: 420px;" onclick="location.href='Cerrar.php';"></div>
...
<?php
	...
	//ESTABLECER POSICION ACTUAL DEL REGISTRO
		$CANTIDAD=mysql_num_rows($funciones->ejecutar("select * from PRODUCTO"));
	$POSICION=$funciones->botones($CANTIDAD);
	//CONSULTA DE TODOS LOS REGISTROS DE LA TABLA PRODUCTO
	...
	//SI LA TABLA ESTA VACIA
		...
		//SI HAY DATOS
			while($datos = mysql_fetch_array($ejecucion)){
				...
				$matriz[$i][0]=$datos['ID_PRODUCTO'];
				$matriz[$i][1]=$datos['NOMBRE'];
				$matriz[$i][2]=$datos['VALOR_UNITARIO'];
...
//ELIMINAR
	if(isset($_POST['BTN_ELIMINAR'])){
		...
		$funciones->ejecutar("DELETE FROM PRODUCTO where  ID_PRODUCTO='$ID_ALMACENADO'");			
...
//MODIFICAR			
	if(isset($_POST['BTN_MODIFICAR'])){
		...
		$VALOR_UNITARIO=TRIM($_POST['TXT_VALOR_UNITARIO']);
		$ID_ALMACENADO=$_POST['TXT_LB_ID'];
		if($VALOR_UNITARIO!="" and $NOMBRE!=""){//VALIDA QUE AYAN DATOS
		
		if(is_numeric($VALOR_UNITARIO)){
		
		$num_rows=mysql_num_rows($funciones->ejecutar("select * from PRODUCTO where NOMBRE='$NOMBRE' and ID_PRODUCTO<>'$ID_ALMACENADO' ")); //VALIDA QUE NO AYA OTRO REGISTRO CON ESOS DATOS
		if($num_rows==0){
			$ejecucion=$funciones->ejecutar("select * from PRODUCTO  where  ID_PRODUCTO='$ID_ALMACENADO'");
			$registros=mysql_num_rows($ejecucion);
			$datos = mysql_fetch_array($ejecucion);
			if($NOMBRE<>$datos['NOMBRE'] or $VALOR_UNITARIO<>$datos['VALOR_UNITARIO']){//VALIDA QUE UNO DE LOS DATOS A MODIFICAR SEA DIFERENTE A LO QUE YA ESTA ALMACENADO
			$funciones->ejecutar("UPDATE PRODUCTO SET NOMBRE=UPPER('$NOMBRE'),VALOR_UNITARIO='$VALOR_UNITARIO' where  ID_PRODUCTO='$ID_ALMACENADO'");  			            $mensaje='REGISTRO MODIFICADO CORRECTAMENTE';
			$imagen='url(../Imagenes/CORRECTO.gif)';?>            
			<script>$('#mensajes').css('display','block');</script>
            <?php 
			}
...
//CIERRA LA VALIDACION DE QUE AYAN DATOS			
		...
		if(isset($_POST['BTN_REGISTRAR'])){
			...
			$VALOR_UNITARIO=TRIM($_POST['TXT_VALOR_UNITARIO']);
			$NOMBRE=TRIM($_POST['TXT_NOMBRE']);
			if($VALOR_UNITARIO!='' and $NOMBRE!=''){
				$num_rows=mysql_num_rows($funciones->ejecutar("select * from PRODUCTO where NOMBRE='$NOMBRE'"));
				if($num_rows==0){
					$numero_registros=mysql_num_rows($funciones->ejecutar("select * from PRODUCTO"));
					...
					$funciones->ejecutar("insert into PRODUCTO(ID_PRODUCTO,NOMBRE,VALOR_UNITARIO) values('$Id_P',UPPER('$NOMBRE'),'$VALOR_UNITARIO')");					
					if($funciones){	  
...
<tr>
	<td width="130" rowspan="5">
       ...
       <input name="BTN_NUEVO" type="button" class="BTN_ESTADO"value="NUEVO"  id="OPCIONNUEVO"  onclick="ocultar();vaciar()"/>
        ...
        <input name="BTN_REGISTRAR" type="submit" class="BTN_GUARDAR" value="REGISTRAR"  id="OPCIONREGISTRAR" disabled="disabled" onclick="requerido_true();"/>
           
         
        <input name="BTN_MODIFICAR" type="button" class="BTN_ESTADO" value="MODIFICAR"  id="OPCIONMODIFICAR"  onclick="requerido_false(); preguntaM(formulario,'<?php echo $matriz[$POSICION][1] ?>')"/>
...
<tr height="35">
    ...
    <td>
    		<input name="TXT_NOMBRE" type="search" id="caja" size="20" Width="290px" class="2" placeholder="NOMBRE" required="required" value="<?php echo $matriz[$POSICION][1]; ?>" />
...
<tr>
<td>
    		<label for="LB_VALOR_UNITARIO" id="label1">VALOR_UNITARIO</label>
             <r>*</r>
    </td>
    <td><input name="TXT_VALOR_UNITARIO" type="number" class="2" id="caja" value="<?php echo $matriz[$POSICION][2]; ?>" size="49"  Width="290px" placeholder="VALOR" required="required" />
    </td>
    <td>
...
?>
<img src="../Imagenes/alerta.gif" style="width: 60px; height: 60px; position: absolute; left: 987px; top: 363px; z-index: 150; cursor: pointer;" onclick="Emergente_Consultar('Consultar.php');<?php $_SESSION['consultar']='Consultar_Producto.php'; ?>" />
<?php
