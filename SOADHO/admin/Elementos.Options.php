...
<?php
		if(!isset($_SESSION['correcto']) ){
			$_SESSION['PAGINA']='Elementos.Options.php';
			?>
...
<div id="ladiz"></div>
    ...
    <div id="cuerpo">
    	<div id="barra_vertical">
         ...
         <div id="area_trabajo">
		 <div id="formulario"><!-- InstanceBeginEditable name="formulario" --><b>Agregar Elementos</b>
		 <div id="session" style="position: absolute; width: 60px; height: 60px; margin-top: -60px; margin-left: -150px; background-image: url(../Imagenes/salir.gif); background-repeat: no-repeat; background-size: 110% 100%; cursor: pointer; left: 227px; top: 402px;" onclick="location.href='Cerrar.php';"></div><!-- InstanceEndEditable --></div>
...
<?php
	...
	//ESTABLECER POSICION ACTUAL DEL REGISTRO
		$CANTIDAD=mysql_num_rows($funciones->ejecutar("select * from ELEMENTO"));
	$POSICION=$funciones->botones($CANTIDAD);
	//CONSULTA DE TODOS LOS REGISTROS DE LA TABLA ELEMENTO
		$ejecucion=$funciones->ejecutar("select * from ELEMENTO");
	...
	//SI LA TABLA ESTA VACIA
		...
		//SI HAY DATOS
			while($datos = mysql_fetch_array($ejecucion)){
				...
				$matriz[$i][0]=$datos['ID_ELEMENTO'];
				$matriz[$i][1]=$datos['NOMBRE'];
				$matriz[$i][2]=$datos['ESTADO'];
				$matriz[$i][3]=$datos['VALOR'];
				$i=$i+1;			
			}
			...
			if(isset($_POST['BTN_BUSCAR'])){
				...
				if ($CONTADOR_BUSQUEDA==0){ 
						$mensaje='NO HAY ELEMENTOS CON ESE NOMBRE!!';
...
//ELIMINAR
	if(isset($_POST['BTN_ELIMINAR'])){
		...
		$FORANEAS=mysql_num_rows($funciones->ejecutar("SELECT * FROM INVENTARIO,ELEMENTO WHERE INVENTARIO.ID_ELEMENTO=ELEMENTO.ID_ELEMENTO AND ELEMENTO.ID_ELEMENTO='$ID_ALMACENADO'"));	
		if($FORANEAS<1){	
		$funciones->ejecutar("DELETE FROM ELEMENTO where  ID_ELEMENTO='$ID_ALMACENADO'");			
...
//MODIFICAR			
	if(isset($_POST['BTN_MODIFICAR'])){
		...
		$ESTADO=TRIM($_POST['CB_ESTADO']);
		$VALOR=TRIM($_POST['TXT_VALOR']);
		$ID_ALMACENADO=$_POST['TXT_LB_ID'];
		if($VALOR<>"" and $NOMBRE<>"" and $ESTADO<>""){//VALIDA QUE AYAN DATOS
		
		$num_rows=mysql_num_rows($funciones->ejecutar("select * from ELEMENTO where NOMBRE='$NOMBRE' and ID_ELEMENTO<>'$ID_ALMACENADO' ")); //VALIDA QUE NO AYA OTRO REGISTRO CON ESOS DATOS
		if($num_rows==0){
			$ejecucion=$funciones->ejecutar("select * from ELEMENTO  where  ID_ELEMENTO='$ID_ALMACENADO'");
			$registros=mysql_num_rows($ejecucion);
			$datos = mysql_fetch_array($ejecucion);
			if($NOMBRE<>$datos['NOMBRE'] or $VALOR<>$datos['VALOR'] or $ESTADO<>$datos['ESTADO']){//VALIDA QUE UNO DE LOS DATOS A MODIFICAR SEA DIFERENTE A LO QUE YA ESTA ALMACENADO
			$funciones->ejecutar("UPDATE ELEMENTO SET NOMBRE=UPPER('$NOMBRE'),VALOR='$VALOR',ESTADO='$ESTADO' where  ID_ELEMENTO='$ID_ALMACENADO'");												            $mensaje='REGISTRO MODIFICADO CORRECTAMENTE';
			$imagen='url(../Imagenes/CORRECTO.gif)';?>            
			<script>$('#mensajes').css('display','block');</script>
            <?php 
			}
...
//CIERRA LA VALIDACION DE QUE AYAN DATOS			
		...
		if(isset($_POST['BTN_REGISTRAR'])){
			...
			$NOMBRE=TRIM($_POST['TXT_NOMBRE']);
			$ESTADO=TRIM($_POST['CB_ESTADO']);
			if($VALOR!='' and $NOMBRE!='' and $ESTADO!=""){
				$num_rows=mysql_num_rows($funciones->ejecutar("select * from ELEMENTO where NOMBRE='$NOMBRE'"));
				if($num_rows==0){
					$numero_registros=mysql_num_rows($funciones->ejecutar("select * from ELEMENTO"));
						if($numero_registros>0){
							$Id_E=$matriz[$CANTIDAD][0];
							$Id_E=$Id_E+1;
						}
						else{
							$Id_E=1;
						}
					$funciones->ejecutar("insert into ELEMENTO values('$Id_E',UPPER('$NOMBRE'),'$ESTADO','$VALOR')");					
					if($funciones){						
						$mensaje='EL ELEMENTO HA SIDO REGISTRADO CORRECTAMENTE';
						$imagen='url(../Imagenes/CORRECTO.gif)';?>     
                        <script>
					...
					$mensaje='POR FAVOR VERIFIQUE, YA EXISTE UN ELEMENTO REGISTRADO CON ESOS DATOS';
						$imagen='url(../Imagenes/error.gif)';?>
						<script>$('#mensajes').css('display','block');</script>
					<?php
...
<tr height="35">
    ...
    <td width="291" >
    		<input name="TXT_BUSCAR" type="search" id="caja"  size="20"  value="" width="290px" class="BUSQUEDA" placeholder="NOMBRE" />
    </td>
    <td width="126">
...
<tr height="35">
    ...
    <td>
    		<input name="TXT_NOMBRE" type="search" id="caja"  value="<?php echo $matriz[$POSICION][1]; ?>" size="20" width="290px" class="2" placeholder="NOMBRE" required="required" />
    </td>
    <td>
...
<tr>
	<td>
    	<label for="LB_ESTADO" id="label1" >ESTADO</label>
        <r>*</r>
    </td>
    <td>
    
      	<select name="CB_ESTADO" onfocus="" class="2" Width="290px" size="1" id="caja" style="width:160px;">
        
           	<option value=""></option>
        	<option class="BE" value="B">BUENO</option>
            <option class="RE" value="R">REGULAR</option>
            <option class="ME" value="M">MALO</option>
...
<tr>
<td>
    		<label for="LB_VALOR" id="label1">VALOR</label>
            <r>*</r>
    </td>
    <td><input name="TXT_VALOR" type="number" class="2" id="caja" value="<?php echo $matriz[$POSICION][3]; ?>" size="49"  Width="290px" placeholder="VALOR" required="required"/>
    </td>
    <td>
...
?>
<img src="../Imagenes/alerta.gif" style="width: 60px; height: 60px; position: absolute; left: 986px; top: 343px; z-index: 150; cursor: pointer;" onclick="Emergente_Consultar('Consultar.php');<?php $_SESSION['consultar']='Consultar_Elemento.php'; ?>" />
<?php
