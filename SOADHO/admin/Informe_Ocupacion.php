...
<?php
		if(!isset($_SESSION['correcto']) ){
			$_SESSION['PAGINA']='Informe_Ocupacion.php';
			?>
...
<div id="ladiz"></div>
    ...
    <div id="cuerpo">
    	<div id="barra_vertical">
         ...
         <div id="area_trabajo">
		 <div id="formulario"><!-- InstanceBeginEditable name="formulario" -->Informe Ocupacion
		  <div id="Categorias">
              <div id="Guardar" onclick="pdf()"></div>
              <div id="Imprimir" onclick="IMP('Contenido1')" style="z-index:100;"></div>
          </div>
		   <!-- InstanceEndEditable --></div>
		 <!-- InstanceBeginEditable name="AREA_TRABAJO" -->
<?php
$funciones->cnx();
	$CANTIDAD=mysql_num_rows($funciones->ejecutar("SELECT HB.CODIGO,HB.DESCRIPCION,BQ.NOMBRE AS BLOQUE, 'LIBRE' AS ESTADO FROM HABITACION HB,BLOQUE BQ WHERE BQ.ID_BLOQUE=HB.ID_BLOQUE AND ID_HABITACION NOT IN(SELECT HB.ID_HABITACION from RESERVA RV, HABITACION HB WHERE HB.ID_HABITACION=RV.ID_HABITACION AND CURDATE() BETWEEN(DATE_FORMAT(RV.FECHA_LLEGADA, '%y/%m/%d'))AND(DATE_FORMAT(RV.FECHA_SALIDA, '%y/%m/%d'))) AND ID_HABITACION NOT IN (SELECT HB.ID_HABITACION from RESERVA RV, HABITACION HB,BLOQUE BQ WHERE BQ.ID_BLOQUE=HB.ID_BLOQUE AND RV.ID_BLOQUE=BQ.ID_BLOQUE AND CURDATE() BETWEEN(DATE_FORMAT(RV.FECHA_LLEGADA, '%y/%m/%d'))AND (DATE_FORMAT(RV.FECHA_SALIDA, '%y/%m/%d')))
UNION
SELECT HB.CODIGO,HB.DESCRIPCION,BQ.NOMBRE AS BLOQUE, 'RESERVADA' AS ESTADO FROM HABITACION HB,BLOQUE BQ WHERE BQ.ID_BLOQUE=HB.ID_BLOQUE AND ID_HABITACION IN(SELECT HB.ID_HABITACION from RESERVA RV, HABITACION HB WHERE HB.ID_HABITACION=RV.ID_HABITACION AND CURDATE() BETWEEN(DATE_FORMAT(RV.FECHA_LLEGADA, '%y/%m/%d'))AND(DATE_FORMAT(RV.FECHA_SALIDA, '%y/%m/%d'))) OR ID_HABITACION IN (SELECT HB.ID_HABITACION from RESERVA RV, HABITACION HB,BLOQUE BQ WHERE BQ.ID_BLOQUE=HB.ID_BLOQUE AND RV.ID_BLOQUE=BQ.ID_BLOQUE AND CURDATE() BETWEEN(DATE_FORMAT(RV.FECHA_LLEGADA, '%y/%m/%d'))AND (DATE_FORMAT(RV.FECHA_SALIDA, '%y/%m/%d')))
GROUP BY 1
ORDER BY 3 ASC"));

$ejecucion=$funciones->ejecutar("SELECT HB.CODIGO,HB.DESCRIPCION,BQ.NOMBRE AS BLOQUE, 'LIBRE' AS ESTADO FROM HABITACION HB,BLOQUE BQ WHERE BQ.ID_BLOQUE=HB.ID_BLOQUE AND ID_HABITACION NOT IN(SELECT HB.ID_HABITACION from RESERVA RV, HABITACION HB WHERE HB.ID_HABITACION=RV.ID_HABITACION AND CURDATE() BETWEEN(DATE_FORMAT(RV.FECHA_LLEGADA, '%y/%m/%d'))AND(DATE_FORMAT(RV.FECHA_SALIDA, '%y/%m/%d'))) AND ID_HABITACION NOT IN (SELECT HB.ID_HABITACION from RESERVA RV, HABITACION HB,BLOQUE BQ WHERE BQ.ID_BLOQUE=HB.ID_BLOQUE AND RV.ID_BLOQUE=BQ.ID_BLOQUE AND CURDATE() BETWEEN(DATE_FORMAT(RV.FECHA_LLEGADA, '%y/%m/%d'))AND (DATE_FORMAT(RV.FECHA_SALIDA, '%y/%m/%d')))
UNION
SELECT HB.CODIGO,HB.DESCRIPCION,BQ.NOMBRE AS BLOQUE, 'RESERVADA' AS ESTADO FROM HABITACION HB,BLOQUE BQ WHERE BQ.ID_BLOQUE=HB.ID_BLOQUE AND ID_HABITACION IN(SELECT HB.ID_HABITACION from RESERVA RV, HABITACION HB,BLOQUE BQ WHERE BQ.ID_BLOQUE=HB.ID_BLOQUE AND HB.ID_HABITACION=RV.ID_HABITACION AND CURDATE() BETWEEN(DATE_FORMAT(RV.FECHA_LLEGADA, '%y/%m/%d'))AND(DATE_FORMAT(RV.FECHA_SALIDA, '%y/%m/%d'))) 
UNION
SELECT HB.CODIGO,HB.DESCRIPCION,BQ.NOMBRE AS BLOQUE, 'RESERVADA' AS ESTADO FROM HABITACION HB,BLOQUE BQ WHERE BQ.ID_BLOQUE=HB.ID_BLOQUE AND ID_HABITACION IN (SELECT HB.ID_HABITACION from RESERVA RV, HABITACION HB,BLOQUE BQ WHERE BQ.ID_BLOQUE=HB.ID_BLOQUE AND RV.ID_BLOQUE=BQ.ID_BLOQUE AND CURDATE() BETWEEN(DATE_FORMAT(RV.FECHA_LLEGADA, '%y/%m/%d'))AND (DATE_FORMAT(RV.FECHA_SALIDA, '%y/%m/%d')))
GROUP BY 1
ORDER BY 1 ASC");
...
$i=1;
	//SI LA TABLA ESTA VACIA
		if($registros<1){
			...
			$matriz[$i][3]=$datos['BLOQUE'];
			$matriz[$i][4]=$datos['ESTADO'];
			$i=$i+1;			
			}
	}
?>		 
...
<div id="Contenido1" style=" width:820px;height:100%;position:absolute;margin:0;margin-left:0%;">
    <div id="Area_Impresion" style="width:720px;background-color:#FFF;margin:0;margin-left:2.5%;margin-top:2.5%;border:2px #000000 solid;border-radius:15px;">
        ...
        <div id="Items" style="background-color:#FFF;width:98.3%;margin:0.5%;margin-top:250px;z-index:5;border:#003300 dashed 1px;border-radius:15px;">   
        	<table width="98%" style="padding:2%;margin-left:1.3%;font-family:Lucida Sans Unicode;color:#000;font-size:13px;">
            	<tr style="width:100%;border:1px solid #F00;background: -webkit-gradient(linear, 0% 0%, 0% 100%, from(#9bbb40), to(#1e5b15));">
                    ...
                    <td><b><center>BLOQUE</center></b></td>
                    <td style="border-top-right-radius:10px;"><b><center>ESTADO</center></b></td>
                </tr>       
        <?php  for($j=1;$j<=$CANTIDAD;$j++){ ?> 
        
        		<tr style="width:100%;border:1px solid #F00;padding-left:2%;">
                	<td class="<?php echo $matriz[$j][4]; ?>" style="padding-left:2%;border:#003300 dashed 1px"><?php echo $matriz[$j][1]; ?></td>
                    <td class="<?php echo $matriz[$j][4]; ?>" style="padding-left:2%;border:#003300 dashed 1px"><?php echo $matriz[$j][2]; ?></td>
                    <td class="<?php echo $matriz[$j][4]; ?>" style="padding-left:2%;border:#003300 dashed 1px"><?php echo $matriz[$j][3]; ?></td>
                    <td class="<?php echo $matriz[$j][4]; ?>" style="padding-left:2%;border:#003300 dashed 1px"><?php echo $matriz[$j][4]; ?></td>
                </tr>
         ...
         <tr style="width:100%;border:1px solid #F00;background: -webkit-gradient(linear, 0% 0%, 0% 100%, from(#9bbb40), to(#1e5b15));height:3%;">
          ...
          <script> $('.LIBRE').css('background-color','#88A0F7');$('.RESERVADA').css('background-color','#FF7171');</script>
        </div>
