...
<div id="TUPLAS" style="background-color:#FFF;width:98.3%;margin:0.5%;margin-top:240px;z-index:2;border:#003300 dashed 1px;border-radius:15px;padding:3px;">          
        <table width="98%" style="padding:1%;margin-left:1.3%;font-family:Lucida Sans Unicode;color:#000;border-collapse: collapse;" id="generar">
            	<tr style="width:100%;background: -webkit-gradient(linear, 0% 0%, 0% 100%, from(#9bbb40), to(#1e5b15));background: -moz-linear-gradient(top, #9bbb40, #1e5b15);">
                	<td style="width:50%;height:50% ;margin-left:1%;border-top-left-radius:15px;text-align:center;">
                    	NOMBRE
                    </td>
                ...
                <?php 
					$ejecucion=$funciones->ejecutar("SELECT CONCAT(PN.NOMBRE,CONCAT(' ',PN.APELLIDO)) AS NOMBRE,CONCAT(PN.TIPO_DOCUMENTO,CONCAT(' ',PN.DNI)) AS DOCUMENTO,LG.NOMBRE AS EXPEDICION FROM PERSONA PN,LUGAR LG WHERE PN.ID_LUGAR=LG.ID_LUGAR");
					$clase=2;
				...
				?>
                    ...
                    <td style="padding-left:2%;border:#003300 dashed 1px"><?php echo $datos['DOCUMENTO']; ?></td>
                    <td style="padding-left:2%;border:#003300 dashed 1px"><?php echo $datos['EXPEDICION']; ?></td>
                    </tr>
                 <?php  
				 	$clase=$clase+1;					
					}
