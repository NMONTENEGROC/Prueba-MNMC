...
$HABITACION_R=$funciones->ejecutar("SELECT  RV.ID_RESERVA,RV.ID_BLOQUE,RV.ID_HABITACION
FROM RESERVA RV,BLOQUE BQ,HABITACION HB
WHERE (RV.ID_BLOQUE=BQ.ID_BLOQUE  OR RV.ID_HABITACION=HB.ID_HABITACION  ) AND RV.ID_RESERVA='$b'
GROUP BY 1");
$ROW_H = mysql_fetch_array($HABITACION_R);
$Habitacion=$ROW_H['ID_HABITACION'];
$Bloque=$ROW_H['ID_BLOQUE'];
		
if($Habitacion!=""){
			$ejecucion=$funciones->ejecutar("select * from HABITACION where ID_HABITACION='$Habitacion'");
			$datos = mysql_fetch_array($ejecucion);
			$Descripcion=$datos['DESCRIPCION'];
			$_SESSION['TIPO_T']="HABITACION";
}
if($Bloque!=""){
		...
		$_SESSION['TIPO_T']="BLOQUE";
}
		
		$ejecucion1=$funciones->ejecutar("SELECT CONCAT(PN.NOMBRE,CONCAT(' ',PN.APELLIDO)) AS NOMBRE_CLIENTE,PN.DNI AS DNI_CLIENTE,PN.TELEFONO AS CELL_CLIENTE,FT.ID_FACTURA,CONCAT(PNL.NOMBRE,CONCAT(' ',PNL.APELLIDO)) AS NOMBRE_RECEPCIONISTA, PNL.DNI AS DNI_RECEPCIONISTA,FT.FECHA,RV.ABONO,FT.TOTAL,DF.VALOR_HABITACION,RV.FECHA_LLEGADA,RV.FECHA_SALIDA
FROM PERSONA PN,RESERVA RV,DETALLE_FACTURA DF,FACTURA FT,PERSONAL PNL
WHERE PN.ID_PERSONA=RV.ID_PERSONA AND RV.ID_RESERVA=DF.ID_RESERVA AND DF.ID_FACTURA=FT.ID_FACTURA AND PNL.ID_PERSONAL=FT.ID_PERSONAL AND RV.ID_RESERVA='$b'");
$datos1 = mysql_fetch_array($ejecucion1);

$F_LLEGADA=date("Y-m-d", strtotime($datos1['FECHA_LLEGADA']));  
$F_SALIDA=date("Y-m-d", strtotime($datos1['FECHA_SALIDA']));  
...
$ITEMS=$funciones->ejecutar("SELECT SV.ID_SERVICIO,SV.CANTIDAD,SV.ID_ELEMENTO,SV.ID_PRODUCTO
FROM SERVICIO SV,PRODUCTO PD,ELEMENTO LM
WHERE (SV.ID_ELEMENTO=LM.ID_ELEMENTO OR SV.ID_PRODUCTO=PD.ID_PRODUCTO) AND SV.ID_FACTURA='$b'
GROUP BY SV.ID_SERVICIO");
$registros_I=mysql_num_rows($ITEMS);
$i=1;
if($registros_I>=1){
	while($ITEMS2 = mysql_fetch_array($ITEMS)){
	$items[$i][0]=$ITEMS2['CANTIDAD'];
	if ($ITEMS2['ID_ELEMENTO']<>NULL){		
					$ID_ELEM=$ITEMS2['ID_ELEMENTO'];
					$ELEMENTO=$funciones->ejecutar("SELECT NOMBRE,VALOR FROM ELEMENTO WHERE ID_ELEMENTO='$ID_ELEM'");
					$ELEMENTO2 = mysql_fetch_array($ELEMENTO);
					$items[$i][1]=$ELEMENTO2['NOMBRE'];
					$items[$i][2]=$ELEMENTO2['VALOR'];
				}
				else{
					$ID_ELEM=$ITEMS2['ID_PRODUCTO'];
					$ELEMENTO=$funciones->ejecutar("SELECT NOMBRE,VALOR_UNITARIO FROM PRODUCTO WHERE ID_PRODUCTO='$ID_ELEM'");
					$ELEMENTO2 = mysql_fetch_array($ELEMENTO);
					$items[$i][1]=$ELEMENTO2['NOMBRE'];
					$items[$i][2]=$ELEMENTO2['VALOR_UNITARIO'];
				}
  ...
  <div id="Area_Impresion" style="width:720px;background-color:#FFF;margin:0;margin-left:2.5%;margin-top:2.5%;border:2px #000000 solid;border-radius:15px;">
    <div id="Header" style="height:18%;background-color:#FFF;color:White ;position:absolute;width:700px;margin-left:1%;margin-top:1%; z-index:2;padding:0;">
        ...
        <div id="Datos_Factura" style="background-color: #FFF; width: 713px; height: 140px; position: absolute; margin-top: 160px; z-index: 1; z-index: 5;">        
            ...
            <input type="text" name="Nombre_Cliente" id="Nombre_Cliente" readonly="readonly" style="width:60%; height:35%; margin-left:35%; border-radius:10px;border:none;" placeholder="Nombre" class="Texto" value="<?php echo $datos1['NOMBRE_CLIENTE']; ?>"/>
            <input type="text" name="DNI_Cliente" value="DNI: <?php echo $datos1['DNI_CLIENTE'];?>" readonly="readonly" style="width:30%; height:35%; margin-left:35%; border-radius:10px;border:none;" placeholder="Documento" class="Texto"/>  
            <input type="text" name="Telefono_Cliente" value="Tel: <?php echo $datos1['CELL_CLIENTE'];?>" readonly="readonly" style="width:28%; height:35%; margin-left:67%; border-radius:10px; margin-top:-18px;border:none;" placeholder="Teléfono"  class="Texto"/>     
            </div>            
            ...
            <input type="text" name="Nombre_Recepcionista" readonly="readonly" style="width:60%; height:35%; margin-left:35%; border-radius:10px;border:none;" placeholder="Nombre" class="Texto" value="<?php echo $datos1['NOMBRE_RECEPCIONISTA']; ?>"/>
            <input name="Documento_Recepcionista" type="text" placeholder="Documento" style="width:30%; height:35%; margin-left:35%; border-radius:10px;border:none;" readonly="readonly" class="Texto" value="DNI: <?php echo $datos1['DNI_RECEPCIONISTA'];?>"/>  
            </div>
            ...
            <input type="text" name="Codigo_Factura" readonly="readonly" style="text-align:right;width:60%; height:50%; margin-left:30%; border-radius:10px; margin-top:10%; border:none;" value="<?php echo $datos1['ID_FACTURA']; ?>" class="Texto"/> 
          </div>          
            ...
            <input type="text" name="Fecha_Factura" readonly="readonly" style="width:60%; height:50%; margin-left:30%; border-radius:10px; margin-top:11%; border:none; text-align:right;" value="<?php echo $datos1['FECHA']; ?>" class="Texto"/>       
            </div>            
        </div>
          	...
          	<table width="98%" style="padding:1%;margin-left:1.3%;font-family:Lucida Sans Unicode;color:#000;font-size:13px;" id="generar">
                ...
                <tr style="width:100%;border:1px solid #F00;padding-left:2%;">
                    ...
                    <td style="padding-right:1%;text-align:right;">
                    	<label for="vlr_hb" id="vlr_hb"><?php echo $VALOR_HABITACION_TOTAL;?> </label> 
                ...
                <?php
					for($i=1;$i<=$registros_I;$i=$i+1){
						?>
                        <tr>
                        	<td><?php echo $items[$i][0].'  Unidade(s)'; ?></td>
                            <td><?php echo $items[$i][1]; ?></td>
                            <td><?php echo '$ '.$items[$i][2]; ?></td>
                            <td><?php echo '$ '.($items[$i][0] * $items[$i][2]); ?></td>
                        </tr>
                        <?php
					}
				?>
        ...
        <div id="Abono" style="background-image: url(../Imagenes/I_Abono.JPG); background-size: 100% 80%; background-repeat: no-repeat; width: 43%; height: 30%; margin: 0.5%; z-index: 3; border: #003300 dashed 1px; padding-left: 5%; border-top-left-radius: 15px; position: absolute;"></div>
          ...
          <div id="TAbono" style="background-color: #FFF; width: 49%; height: 30%; margin: 0.5%; z-index: 3; border: #003300 dashed 1px; border-top-right-radius: 15px; margin-left: 50%; position: absolute;"><label id="Txt_Abono" for="Txt_Abono"><?php echo ' $'.$datos1['ABONO']; ?></label></div>
          ...
          <div id="Valor_Subtotal" style="background-color: #FFF; width: 49%; height: 30%; margin: 0.5%; z-index: 3; border: #003300 dashed 1px;  margin-left: 50%; position: absolute;margin-top: 15%;"><label id="Txt_Valor_Subtotal" for="Txt_Valor_Subtotal"><?php echo ' $'.(($datos1['TOTAL'])-($datos1['TOTAL']*0.16)) ?></label></div>
          ...
          <div id="Valor_Iva" style="background-color: #FFF; width: 49%; height: 30%; margin: 0.5%; z-index: 3; border: #003300 dashed 1px; margin-left: 50%; position: absolute; margin-top: 28%;"><label id="Txt_Valor_Iva" for="Txt_Valor_Iva"><?php echo ' $'.$datos1['TOTAL']*0.16 ?></label></div>
